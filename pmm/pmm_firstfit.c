...
static void firstfit_init(void) {
  list_init(&free_list);
  nr_free = 0;
}

static void firstfit_init_memmap(struct Page *base, size_t n) {
  assert(n > 0);
  struct Page *p = base;
  for (; p != base + n; p++) {
...
static struct Page *firstfit_alloc_pages(size_t n) {
  assert(n > 0);
  if (n > nr_free) {
    return NULL;
  }
...
static void firstfit_free_pages(struct Page *base, size_t n) {
  assert(n > 0);
  struct Page *p = base;
  for (; p != base + n; p++) {
...
static size_t firstfit_nr_free_pages(void) { return nr_free; }
